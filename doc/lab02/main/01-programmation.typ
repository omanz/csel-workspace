...
Une fonctionnalité optionnelle permet l'incrémentation/décrémentation automatique de la fréquence de clignotement lors d’un appui prolongé sur les boutons K1 et K3.
...
La deuxième partie du laboratoire (Multiprocessing et Ordonnanceur) permet d'exercer la communication interprocessus, la gestion des signaux et le contrôle des ressources à l’aide des CGroups à l'aide de plusieurs exercices.
...
...
```
Nous ajoutons la gestion des boutons à l'aide du multiplexage des entrées et sorties avec la fonction `epoll()`.
...
...
```
Nous ajoutons également la fonctionnalité automatique d'incrémentation/décrémentation de la fréquence de clignotement lors du maintien des boutons K1 ou K3. Nous créons, pour cela, un timer supplémentaire pour détecter le maintien d'un bouton, et nous ajustons la fréquence de clignotement en conséquence. La période de ce timer est hardcodée, la valeur est choisie de manière empirique. Cette méthode n'est pas optimale mais elle permet d'avoir une bonne réactivité sans être trop rapide.
...
#questionbox()[1. Quel effet a la commande echo $$ > ... sur les cgroups ?]
...
#questionbox()[2. Quel est le comportement du sous-système memory lorsque le quota de mémoire est épuisé ? Pourrait-on le modifier ? Si oui, comment ?]
...
Si la nouvelle limite est inférieure à la consommation actuelle, la commande échoue avec une erreur `Device or resource busy`.
...
Pour désactiver l'OOM-killer, nous pouvons écrire `1` sur le fichier `memory.oom_control`
...
#questionbox()[3. Est-il possible de surveiller/vérifier l’état actuel de la mémoire ? Si oui, comment ?]
Pour connaître l'utilisation de la mémoire de notre sous groupe nous utilisons la commande `# cat /sys/fs/cgroup/memory/mem/memory.usage_in_bytes`
...
```
#questionbox()[1. Les 4 dernières lignes sont obligatoires pour que les prochaines commandes fonctionnent correctement. Pouvez-vous en donner la raison ?]
...
Certains systèmes linux peuvent avoir leur mémoire divisée en “noeuds” mais dans notre cas, nous disposons d'un seul noeud mémoire.
...
```
#pagebreak()
#questionbox()[2. Ouvrez deux shells distincts et placez une dans le cgroup high et l’autre dans le cgroup low. Lancez ensuite votre application dans chacun des shells. Quel devrait être le bon comportement ? Pouvez-vous le vérifier ?]
...
#pagebreak()
#questionbox()[3. Sachant que l’attribut cpu.shares permet de répartir le temps CPU entre différents cgroups, comment devrait-on procéder pour lancer deux tâches distinctes sur le cœur 4 de notre processeur et attribuer 75% du temps CPU à la première tâche et 25% à la deuxième ?]
...
Nous avons constaté que en écrivant la valeur "1" dans "cpu.shares", celle-ci est forcée à "2". Nous adaptons les valeurs en conséquence tout en conservant le même ratio.
...
Le fait de nous avoir donné une base de programme fonctionnelle, puis de nous demander d'ajouter des fonctionnalités tout en l'optimisant, a été une très bonne approche pour comprendre le fonctionnement global du système. Selon nous, une structure similaire pour les laboratoires précédents aurait représenté un avantage pédagogique, malgré la charge de travail supplémentaire qu’elle implique.
